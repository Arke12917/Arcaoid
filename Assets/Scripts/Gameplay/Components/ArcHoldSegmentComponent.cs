...
{
    public class ArcHoldSegmentComponent : MonoBehaviour
    {
        ...
        void OnEnable()
        {
            ...
            lerpyrunning = false;
                ...
                AAR = transform.GetComponentInParent<ArcHoldRenderer>();
            if (!(ReferenceEquals(AAR, null)))
            {
                if (!(ReferenceEquals(AAR.ActualCap, null)))
                    MVCAP = AAR.ActualCap.GetComponent<movecaphold>();
            }
        ...
        private void Update()
        {
            ...
            if (-FromPos.z < z+200 && -ToPos.z >= z-200 && !ArcTimingManager.Instance.IsStopped && !ArcTimingManager.Instance.IsBackwarding&&Time.timeScale!=0)
            {
                ...
                if (AAR.ArcHold.IsVoid)               
                {
                    From = ongoingfrom;
                }
                else if(shrinkrender||inposition)
                {
                    if (!wasfrom)
                    {
                        wasfrom = true;
