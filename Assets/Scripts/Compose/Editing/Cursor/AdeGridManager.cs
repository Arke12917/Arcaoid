...
namespace Arcaoid.Compose
{
    ...
    public class AdeGridManager : MonoBehaviour, IMarkingMenuItemProvider
    {
        ...
        public string PreferencesSavePath
        {
            get
            {
                return ArcaoidComposeManager.ArcadePersistentFolder + "/Grid.json";
            }
        }
        ...
        private void CalculateBeatlineTimes()
        {
            ...
            {
                float segment = Timings[i].Bpm == 0 ? (Timings[i + 1].Timing - Timings[i].Timing) : (60000 / Mathf.Abs(Timings[i].Bpm) / BeatlineDensity);
            ...
            if (Timings.Count >= 1)
            {
              ...
              : 60000 / Mathf.Abs(Timings[Timings.Count - 1].Bpm) / BeatlineDensity;
                if (segmentRemain != 0)
                {
                    int primaryCount = 0;
                    int n = 0;
        ...
        private void LoadCustomGrids()
        {
            try
            {
                customGrid = JsonConvert.DeserializeObject<CustomGrid>(File.ReadAllText(PreferencesSavePath));
            }
            catch (Exception)
            {
        ...
        {
            File.WriteAllText(PreferencesSavePath, JsonConvert.SerializeObject(customGrid));
        }
